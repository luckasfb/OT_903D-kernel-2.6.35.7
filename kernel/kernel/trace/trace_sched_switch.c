...
void
tracing_set_sched(struct trace_array *tr, struct task_struct *tsk, unsigned char policy,
			   unsigned long flags, int pc)
{
	//struct ftrace_event_call *call = &event_setsched;
	struct ring_buffer_event *event;
	struct set_sched_entry *entry;
	struct ring_buffer *buffer = tr->buffer;

	event = trace_buffer_lock_reserve(buffer, TRACE_MTK_SET_SCHED,
					  sizeof(*entry), flags, pc);
	if (!event)
		return;
	entry	= ring_buffer_event_data(event);
...
static void
probe_set_sched(void *ignore, struct task_struct *tsk, unsigned char policy)
{
	struct trace_array_cpu *data;
	unsigned long flags;
	int cpu, pc;
	...
	if (likely(!atomic_read(&data->disabled)))
		tracing_set_sched(ctx_trace, tsk ,policy
					   ,flags, pc);
...
#ifdef CONFIG_MTK_SCHED_TRACERS
	ret = register_trace_set_sched(probe_set_sched, NULL);
	if (ret) {
		pr_info("sched trace: Couldn't activate tracepoint"
			" probe to kernel_set_sched\n");
		goto fail_deprobe_trace_set_sched;
	}
