...
void (*set_intact_mode)(void);
EXPORT_SYMBOL(set_intact_mode);

void set_sched_latency(unsigned int ltcy, unsigned int min_gr)
{
	int factor = get_update_sysctl_factor();
	sysctl_sched_latency = ltcy;
	sysctl_sched_min_granularity = min_gr; 
	sched_nr_latency = DIV_ROUND_UP(sysctl_sched_latency,
					sysctl_sched_min_granularity);
#define WRT_SYSCTL(name) \
	(normalized_sysctl_##name = sysctl_##name / (factor))
...
EXPORT_SYMBOL(set_sched_latency);
