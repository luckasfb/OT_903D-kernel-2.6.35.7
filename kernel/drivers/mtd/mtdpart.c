...
static int init_mtd_writeable = 0;
static int mtd_writeable_proc_write(struct file *file, const char *buffer, unsigned long count, void *data);

struct mtd_change {
    uint64_t size;
    uint64_t offset;
};
static int mtd_change_proc_write(struct file *file, const char *buffer, unsigned long count, void *data);
#endif
...
#ifdef DYNAMIC_CHANGE_MTD_WRITEABLE //wschen 2011-01-05
        if (init_mtd_writeable == 0) {
            struct proc_dir_entry *entry;
            init_mtd_writeable = 1;
            entry = create_proc_entry("driver/mtd_writeable", 0666, NULL);
            if (entry != NULL) {
                entry->write_proc = mtd_writeable_proc_write;
            }

            entry = create_proc_entry("driver/mtd_change", 0666, NULL);
            if (entry != NULL) {
...
#define MTD_CHANGE_NUM 3
static int mtd_change_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    struct mtd_change mtd_change[MTD_CHANGE_NUM];

    if (count != (sizeof(struct mtd_change) * MTD_CHANGE_NUM)) {
        return -EFAULT;
    }
