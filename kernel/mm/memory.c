...
#ifdef MT6573M4U_DBG
#define M4UDBG(string, args...) printk("[M4U_K]"string,##args)
#define M4U_ASSERT(x) ASSERT(x);
#else
#define M4UDBG(string, args...)
#define M4U_ASSERT(x) if(!(x)){ \
	printk("[M4U_K] assert fail, file:%s, line:%d \n", __FILE__, __LINE__);}
#endif

// for error msg, high priority
#define MT6573M4U_MSG
#ifdef MT6573M4U_MSG
#define M4UMSG(string, args...) printk("[M4U_K]"string,##args)
#else
                     ...
                     struct page **pages, struct vm_area_struct **vmas)
{
        ...
        do { 
                ...
                {
                	  M4UMSG("m4u_get_pages(): bypass pmem garbage pages!\n");
                	  return PFNMAP_FLAG_SET;
                }                     
                if(vma->vm_flags & VM_IO)
                {
                	  M4UMSG("warning: vma is marked as VM_IO \n");
                }
...
void mfv_make_pmem_to_noncached(unsigned int PmemVAddr_from_user)
{
    struct vm_area_struct *vma;
    unsigned int PmemVAddr;

    PmemVAddr = PmemVAddr_from_user;
...
EXPORT_SYMBOL(mfv_make_pmem_to_noncached);
