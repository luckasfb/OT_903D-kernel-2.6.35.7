...
parser.add_option("-W","--wine",action="store_true",help="check requirement for wine")
...
optionsList.append({"wine":options.wine})
    ...
    def checkMakeVersion(self):
        ...
        if match:
            ...
            if int(number[0]) < 3 or (int(number[0]) == 3 and int(number[1]) < 81):
               ...
               self.flag = "WARNING"
               self.info = "your make version is higher than recommendation"
...
class JavaCheck(object):
    ...
    def checkInstall(self):
        returnCode,self.java = commands.getstatusoutput("which java")
        if returnCode != 0:
    ...
    def checkEabiVersion(self):
        ...
        if match:
            ...
            if int(number[0]) < 4 \
               or (int(number[0]) == 4 and int(number[1]) < 4) \
               or (int(number[0]) == 4 and int(number[1]) == 4 and int(number[2]) < 3):
                self.info = "your arm-eabi-gcc version is lower than recommendation"
            elif int(number[0]) == 4 and int(number[1]) == 4 and int(number[2]) == 3:
               self.flag = "OK"
            else:
               self.flag = "WARNING"
               self.info = "your arm-eabi-gcc version is higher than recommendation"
        else: self.versionNo = "unknown"
...
class WineCheck(object):
    """ check the requirement for wine """
    def __init__(self):
        self.flag = "FAIL"
        self.tag = True
        self.info = ""
    ...
    def checkEnv(self):
        self.checkInstall()
        if self.tag:
            self.checkWineVersion()

    def checkInstall(self):
        returnCode,self.wine = commands.getstatusoutput("which wine")
        if returnCode != 0:
            self.info = "you have not installed wine"
            self.tag = False
            result("wine","","",self.flag,self.info)
            global checkResult
            checkResult = 1

    def checkWineVersion(self):
        wineVersion = commands.getoutput("%s --version" % self.wine)
        pattern = re.compile("wine\s*-\s*([\d\.]+)",re.S)
        match = pattern.match(wineVersion)
        if match:
            self.versionNo = match.group(1)
            number = self.versionNo.split(".")
            if int(number[0]) < 1 or (int(number[0]) == 1 and int(number[1]) < 1):
                self.info = "your wine version is lower than recommendation"
            elif int(number[0]) == 1 and int(number[1]) == 1:
               self.flag = "OK"
            else:
               self.flag = "WARNING"
               self.info = "your wine version is higher than recommendation"
        else: self.versionNo = "unknown"
        wineBit = commands.getoutput("file -bL %s" % self.wine)
        pattern = re.compile("ELF\s*(\d+)-bit\s*LSB\s*executable.*")
        match = pattern.match(wineBit)
        if match:
            self.bit = match.group(1)
        else:
            self.bit = "unknown"
        result("wine",self.versionNo,self.bit,self.flag,self.info)
        if self.flag == "FAIL":
            global checkResult
            checkResult = 1
...
for item in optionsList:
    if item.has_key("os") and item.get("os") == True:
        ...
        e = EabiCheck()
        e.checkEnv()
    elif item.has_key("wine") and item.get("wine") == True:
        w = WineCheck()
        w.checkEnv()
