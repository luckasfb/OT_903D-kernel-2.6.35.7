...
{
     SENSORDB("[Enter]:MT9V114 Open func zhijie:111");
...
static kal_uint32 MT9V114_GetSensorID(kal_uint32 *sensorID)
...
void MT9V114_night_mode(kal_bool enable)
{
	...
	if(enable)
	{
	    MT9V114_write_cmos_sensor(0x098E, 0xA076);// MCU_ADDRESS [AE_MAX_INDEX]
    ...
    {
		MT9V114_write_cmos_sensor(0x098E, 0xA076);  
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	pSensorInfo->SensorOutputDataFormat=SENSOR_OUTPUT_FORMAT_YUYV;    //YUYV;
...
static void MT9V114_set_AE_mode(kal_bool AE_enable)
{
...
static void MT9V114_set_AWB_mode(kal_bool AWB_enable)
{
	kal_uint8 temp_AE_reg = 0;
...
BOOL MT9V114_set_param_wb(UINT16 para)
{
   ...
   SENSORDB("[Enter]MT9V114 set_param_wb func:para = %d\n",para);
	switch (para)
	{   case AWB_MODE_OFF:
            MT9V114_set_AWB_mode(KAL_FALSE);
            break;        
		case AWB_MODE_AUTO:
            MT9V114_set_AWB_mode(KAL_TRUE);
...
UINT32 MT9V114YUVSensorSetting(FEATURE_ID iCmd, UINT32 iPara)
{
	...
	case FID_AE_SCENE_MODE: 
		if (iPara == AE_MODE_OFF) {
			...
			MT9V114_set_AE_mode(KAL_FALSE);
			}
			else {
				MT9V114_set_AE_mode(KAL_TRUE);
			}
		break; 
	default:
	     break;
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_CHECK_SENSOR_ID:
			MT9V114_GetSensorID(pFeatureData32); 
			break; 	
		default:
			 break;			
	}
