...
void HM3451_sensor_to_camera_para(void)
{
  ...
  for(i=0; 0xFFFFFFFF!=HM3451SensorReg[i].Addr; i++)
  {
      HM3451SensorReg[i].Para = HM3451_read_cmos_sensor(HM3451SensorReg[i].Addr);
  }
  for(i=ENGINEER_START_ADDR; 0xFFFFFFFF!=HM3451SensorReg[i].Addr; i++)
  {
      HM3451SensorReg[i].Para = HM3451_read_cmos_sensor(HM3451SensorReg[i].Addr);
...
{   kal_uint8 SensorVerson = 0;
	...
	HM3451_write_cmos_sensor(0x0036,0xff);   //0xfc           //0xcc -> 0xfc: For Driving Current
	...
	HM3451_write_cmos_sensor(0x0040,0x49);				//Analog Block: 
	...
	HM3451_write_cmos_sensor(0x004B,0x24); 
	HM3451_write_cmos_sensor(0x004C,0x44); 
	HM3451_write_cmos_sensor(0x004D,0x09); 
	HM3451_write_cmos_sensor(0x004E,0x0A); 
	HM3451_write_cmos_sensor(0x004F,0x0B); 
	HM3451_write_cmos_sensor(0x0050,0x0C);
	
	HM3451_write_cmos_sensor(0x0051,0x04);				//0x2C -> 0x04  
	HM3451_write_cmos_sensor(0x0052,0x24);				//0x4C -> 0x24   
	HM3451_write_cmos_sensor(0x0053,0x44);				//0xCC -> 0x44   
	HM3451_write_cmos_sensor(0x0054,0xC4);				//0xC4 -> 0xCC -> 0xC8 ->  0xC4: For Low Light Capture Blue
	...
	HM3451_write_cmos_sensor(0x0066,0x0F); 
	HM3451_write_cmos_sensor(0x0067,0x25); 
...
{ // full size: 2048*1536
	...
	HM3451_write_cmos_sensor(0x000F,0x1e);			//0x1A -> 0x00	
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	switch (ScenarioId)
	{
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_MEM:
			...
			pSensorInfo->SensorGrabStartY = HM3451_CP_START_Y+20;			
		break;
		default:
			pSensorInfo->SensorClockFreq=52;//48;
			pSensorInfo->SensorClockRisingCount=0;
