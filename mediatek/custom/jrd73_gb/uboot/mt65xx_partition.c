...
static part_dev_t *mt6573_part_dev;
...
static uchar *mt6573_part_buf;

void mt6573_part_dump(void)
{
...
void mt6573_part_init(unsigned long totalblks)
{
    part_t *part = &mt6573_parts[0];
    unsigned long lastblk;

    mt6573_part_buf = (uchar*)malloc(BLK_SIZE * 2);
...
int mt6573_part_generic_read(part_dev_t *dev, ulong src, uchar *dst, int size)
{
    int dev_id = dev->id;
    uchar *buf = &mt6573_part_buf[0];
...
static int mt6573_part_generic_write(part_dev_t *dev, uchar *src, ulong dst, int size)
{
    int dev_id = dev->id;
    uchar *buf = &mt6573_part_buf[0];
    block_dev_desc_t *blkdev = dev->blkdev;
...
int mt6573_part_register_device(part_dev_t *dev)
{
    if (!mt6573_part_dev) {
        if (!dev->read)
            dev->read = mt6573_part_generic_read;
        if (!dev->write)
            dev->write = mt6573_part_generic_write;
        mt6573_part_dev = dev;

        #ifdef CFG_NAND_BOOT
        if (dev->id == CFG_MT6573_NAND_ID)
	{
#ifdef PMT
		...
		part_init_pmt(BLK_NUM(1 * GB),dev);
#else
            mt6573_part_init((unsigned long)dev->blkdev->lba);
#endif /* CFG_NAND_BOOT */
        }
	#endif /* CFG_NAND_BOOT */
    }
...
part_dev_t *mt6573_part_get_device(void)
{
    if (mt6573_part_dev && !mt6573_part_dev->init && mt6573_part_dev->init_dev) 
    {
        mt6573_part_dev->init_dev(mt6573_part_dev->id);
        mt6573_part_dev->init = 1;
    }
    return mt6573_part_dev;
}
