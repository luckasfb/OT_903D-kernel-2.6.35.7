        ...
        case MATV_SLEEP:
            {
                ...
                do_gettimeofday(&t1);
                if(0 != wait_event_interruptible_timeout(matvWaitQueue, NULL, timeout_jiff))
                    MATV_LOGE("[MATV] Fail to sleep enough time\n");
                do_gettimeofday(&t2);
...
static int matv_release(struct inode *inode, struct file *file)
{
    ...
    //Disable I2D Data pin
    //mt_set_gpio_mode(GPIO_MATV_I2S_DATA,GPIO_MODE_01);
    //mt_set_gpio_pull_enable(GPIO_MATV_I2S_DATA,false);
    //mt_set_gpio_out(GPIO_MATV_I2S_DATA, GPIO_OUT_ZERO);    
    //~    
