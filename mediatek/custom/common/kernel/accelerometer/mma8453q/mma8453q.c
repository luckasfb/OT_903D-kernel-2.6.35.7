...
{    
	...
	if(err = hwmsen_read_byte_sr(client, MMA8453Q_REG_OFSX, &ofs[MMA8453Q_AXIS_X]))
	{
		GSE_ERR("error: %d\n", err);
	}
	if(err = hwmsen_read_byte_sr(client, MMA8453Q_REG_OFSY, &ofs[MMA8453Q_AXIS_Y]))
	{
		GSE_ERR("error: %d\n", err);
	}
	if(err = hwmsen_read_byte_sr(client, MMA8453Q_REG_OFSZ, &ofs[MMA8453Q_AXIS_Z]))
	{
		GSE_ERR("error: %d\n", err);
	}
...
static int mma8453q_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = mma8453q_create_attr(&mma8453q_gsensor_driver.driver))
	{
...
static int mma8453q_i2c_remove(struct i2c_client *client)
{
	...
	if(err = mma8453q_delete_attr(&mma8453q_gsensor_driver.driver))
	{
...
/*----------------------------------------------------------------------------*/
static int mma8453q_probe(struct platform_device *pdev) 
{
...
/*----------------------------------------------------------------------------*/
static int mma8453q_remove(struct platform_device *pdev)
{
    struct acc_hw *hw = get_cust_acc_hw();
