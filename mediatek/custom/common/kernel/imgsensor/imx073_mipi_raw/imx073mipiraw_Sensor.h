
/* SENSOR FULL SIZE */
#ifndef __SENSOR_H
#define __SENSOR_H

typedef enum group_enum {
    PRE_GAIN=0,
    CMMCLK_CURRENT,
    FRAME_RATE_LIMITATION,
    REGISTER_EDITOR,
    GROUP_TOTAL_NUMS
} FACTORY_GROUP_ENUM;


#define ENGINEER_START_ADDR 10
#define FACTORY_START_ADDR 0

typedef enum engineer_index
{
    CMMCLK_CURRENT_INDEX=ENGINEER_START_ADDR,
    ENGINEER_END
} FACTORY_ENGINEER_INDEX;



typedef enum register_index
{
    PRE_GAIN_INDEX=FACTORY_START_ADDR,
    GLOBAL_GAIN_INDEX,
    FACTORY_END_ADDR
} FACTORY_REGISTER_INDEX;

typedef struct
{
    SENSOR_REG_STRUCT	Reg[ENGINEER_END];
    SENSOR_REG_STRUCT	CCT[FACTORY_END_ADDR];
} SENSOR_DATA_STRUCT, *PSENSOR_DATA_STRUCT;


//Macro for Resolution
#define IMX073MIPI_IMAGE_SENSOR_CCT_WIDTH     (3284)
#define IMX073MIPI_IMAGE_SENSOR_CCT_HEIGHT    (2462)

/* SENSOR 720P SIZE */
#define IMX073MIPI_IMAGE_SENSOR_PV_WIDTH   (820) //(1600)//(3272-8)
#define IMX073MIPI_IMAGE_SENSOR_PV_HEIGHT  (612) //(1200)//(612)

/* SENSOR 8M SIZE */
#define IMX073MIPI_IMAGE_SENSOR_FULL_WIDTH                                       3284
#define IMX073MIPI_IMAGE_SENSOR_FULL_HEIGHT                                      2462

#define IMX073MIPI_IMAGE_SENSOR_PV_STARTX                         4
#define IMX073MIPI_IMAGE_SENSOR_PV_STARTY                         2 //  The value must bigger or equal than 1

/* SENSOR PIXEL/LINE NUMBERS IN ONE PERIOD */
#define IMX073MIPI_FULL_PERIOD_PIXEL_NUMS  (3284)  // 8M mode's pixel # in one HSYNC w/o dummy pixels
#define IMX073MIPI_FULL_PERIOD_LINE_NUMS   (2462)  // 8M mode's HSYNC # in one HSYNC w/o dummy lines
#define IMX073MIPI_PV_PERIOD_PIXEL_NUMS    (820)//(1600)//(3272-8)    // 720P mode's pixel # in one HSYNC w/o dummy pixels
#define IMX073MIPI_PV_PERIOD_LINE_NUMS    (612) //(1200)//(612)   // 720P mode's HSYNC # in one HSYNC w/o dummy lines

#define IMX073MIPI_FULL_PERIOD_EXTRA_PIXEL_NUMS 16
#define IMX073MIPI_FULL_PERIOD_EXTRA_LINE_NUMS 14
#define IMX073MIPI_PV_PERIOD_EXTRA_PIXEL_NUMS 4 //32
#define IMX073MIPI_PV_PERIOD_EXTRA_LINE_NUMS 17 //44 //500

#define IMX073MIPI_IMAGE_SENSOR_8M_PIXELS_LINE         3292
#define IMX073MIPI_IMAGE_SENSOR_720P_PIXELS_LINE      1716 // 820 //1600 // 616

#define MAX_FRAME_RATE	(15)
#define MIN_FRAME_RATE  (12)

/* SENSOR EXPOSURE LINE LIMITATION */
#define IMX073MIPI_FULL_EXPOSURE_LIMITATION   (2496) // 8M mode
#define IMX073MIPI_PV_EXPOSURE_LIMITATION (612 + 490) // (1254)  // # of lines in one 720P frame
// SENSOR VGA SIZE
//For 2x Platform camera_para.c used
#define IMAGE_SENSOR_PV_WIDTH    IMX073MIPI_IMAGE_SENSOR_PV_WIDTH
#define IMAGE_SENSOR_PV_HEIGHT   IMX073MIPI_IMAGE_SENSOR_PV_HEIGHT

#define IMAGE_SENSOR_FULL_WIDTH	  IMX073MIPI_IMAGE_SENSOR_FULL_WIDTH
#define IMAGE_SENSOR_FULL_HEIGHT	  IMX073MIPI_IMAGE_SENSOR_FULL_HEIGHT

#define IMX073MIPI_SHUTTER_LINES_GAP	  3


#define IMX073MIPI_WRITE_ID (0x34)
#define IMX073MIPI_READ_ID	(0x35)

// SENSOR CHIP VERSION

#define IMX073MIPI_SENSOR_ID            IMX073_SENSOR_ID

#define IMX073MIPI_PAGE_SETTING_REG    (0xFF)

//s_add for porting
//s_add for porting
//s_add for porting

//export functions
UINT32 IMX073MIPIOpen(void);
UINT32 IMX073MIPIGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution);
UINT32 IMX073MIPIGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_INFO_STRUCT *pSensorInfo, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData);
UINT32 IMX073MIPIControl(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData);
UINT32 IMX073MIPIFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId, UINT8 *pFeaturePara,UINT32 *pFeatureParaLen);
UINT32 IMX073MIPIClose(void);

//#define Sleep(ms) mdelay(ms)
//#define RETAILMSG(x,...)
//#define TEXT

//e_add for porting
//e_add for porting
//e_add for porting

#endif /* __SENSOR_H */

