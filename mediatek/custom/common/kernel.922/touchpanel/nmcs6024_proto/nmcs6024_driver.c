...
static struct touch_info cinfo, sinfo;



extern struct tpd_device *tpd;
extern int tpd_show_version;
extern int __flag_first;
...
static const struct i2c_device_id tpd_i2c_id[] = {{"mtk-touch",0},{}};
...
//	char mcs_buf[8]={0};
	...
	if (__flag_first)
	{
		printk("@@@@@@@@@@flag_first!Mstar has been installed already?!\n");
		return -1;
	}
...
static int tpd_gettouchinfo(struct touch_info *cinfo) {
	uint8_t status = 0x10;
...
static int touch_event_handler(void *unused) {
    ...
    do {
	 ...
	 if((tpd_gettouchinfo(&cinfo)) && (sinfo.TouchpointFlag==0) && ((sinfo.VirtualKeyFlag & 0x03) == 0)) 
		 continue; 
...
void tpd_touch_suspend(struct early_suspend *h)
{
	//if(tpd_debuglog==1) {
		//TPD_DMESG("[mtk-tpd] %s\n", __FUNCTION__); 
	//}
...
void tpd_touch_resume(struct early_suspend *h) 
{
...
static struct tpd_driver_t tpd_touch_device_driver = {
		.tpd_device_name = "mcs6024",
		.tpd_local_init = tpd_touch_local_init,
		.suspend = tpd_touch_suspend,
		.resume = tpd_touch_resume,
#ifdef TPD_HAVE_BUTTON
		.tpd_have_button = 1,
#else
		.tpd_have_button = 0,
...
static int __init tpd_driver_init(void) {
    printk("MediaTek mcs6024 touch panel driver init\n");
		if(tpd_driver_add(&tpd_touch_device_driver) < 0)
...
static void __exit tpd_driver_exit(void) {
    ...
    tpd_driver_remove(&tpd_touch_device_driver);
}
