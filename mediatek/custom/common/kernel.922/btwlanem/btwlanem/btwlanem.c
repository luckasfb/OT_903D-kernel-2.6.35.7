...
#define mt65xx_eint_set_hw_debounce(eintno, ms) MT6516_EINT_Set_HW_Debounce(eintno, ms)
#define mt65xx_eint_set_sens(eintno, sens) MT6516_EINT_Set_Sensitivity(eintno, sens)
#define mt65xx_eint_registration(eintno, Dbounce_En, ACT_Polarity, EINT_FUNC_PTR, auto_umask) \
        MT6516_EINT_Registration(eintno, Dbounce_En, ACT_Polarity, EINT_FUNC_PTR, auto_umask)
...
{
    eint_gen = 0;    
    BTWLAN_EM_DEBUG("btwaln_em_open\n");
    return 0;
}

static int btwlan_em_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
    BTWLAN_EM_DEBUG("[BTEM] +btwaln_em_ioctl\n");    
    if(!pbtwlan_em)
    {
        BTWLAN_EM_ALERT("btwaln_em_ioctl failed get valid struct\n");
        return -EFAULT;
    }
    ...
    switch(cmd)
    {
        case BTWLAN_EM_IOCTL_SET_BTPWR:  
        {
            ...
            BTWLAN_EM_DEBUG("[BTEM]  BTWLAN_EM_IOCTL_SET_BTPWR:%d\n", (int)btpwr); 
        ...
        default:
            BTWLAN_EM_ALERT("btwaln_em_ioctl not support\n");
            return -EPERM;
    }
    
    BTWLAN_EM_DEBUG("-btwaln_em_ioctl\n");
    return 0;
}

static int btwlan_em_release(struct inode *inode, struct file *file)
{
    BTWLAN_EM_DEBUG("[BTEM]  btwaln_em_release\n");
    return 0;
}
...
static unsigned int btwlan_em_poll(struct file *file, poll_table *wait)
{
    ...
    //BTWLAN_EM_DEBUG("+btwlan_em_poll:%d\n", eint_gen); 
    poll_wait(file, &eint_wait, wait); 
    //wait_event_interruptible(eint_wait, (eint_gen == 1));
    //BTWLAN_EM_DEBUG("-btwlan_em_poll:%d\n", eint_gen); 
    if(eint_gen == 1)
    { 
...
static int __init btwlan_em_init(void)
{
    ...
    {
        BTWLAN_EM_ALERT("[BTEM] btwlan_em_init allocate dev struct failed\n");
        err = -ENOMEM;
        goto ERR_EXIT;
    }
