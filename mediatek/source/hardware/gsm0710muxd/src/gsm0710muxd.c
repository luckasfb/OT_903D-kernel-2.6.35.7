...
void* malloc_r( int size )
{
    ...
    {
        if ( (mem = malloc( size )) != NULL ) return mem;
        else {
...
static int logical_channel_close(
	Channel* channel)
{
    if (channel->fd >= 0)
    {   
...
static int logical_channel_init(
	Channel* channel, 
	int id)
{
    int i;
...
static int logical_channel_establish( 
	Channel* channel )
{  
    if ( setup_pty_interface( channel ) != 0 )
    {
        ...
        LOGMUX(LOG_INFO, "Connecting %s to virtual channel %d on %s",
            channel->ptsname, channel->id, serial.devicename);              
    }
...
static int setup_pty_interface(
	Channel* channel )
{
    ...
    //create thread
    LOGMUX(LOG_DEBUG, "New channel properties: number: %d fd: %d device: %s", channel->id, 
        channel->fd, channel->devicename);
	...
	GSM0710_Frame * frame)
{
    if (frame->length > 0)
    {
        LOGMUX(LOG_DEBUG,"frame_data ptr=0x%02X, frame_len=%d", 
            (unsigned int) frame->data, frame->length);            
	...
	GSM0710_Buffer* buf)
{
    ...
    while ((frame = cmux_mode 
		...
		: gsm0710_base_buffer_get_frame(buf,0)))
    {
        ...
        if ((GSM0710_FRAME_IS(GSM0710_TYPE_UI, frame) || GSM0710_FRAME_IS(GSM0710_TYPE_UIH, frame)))
        {
            ...
            if (frame->channel > 0)
            {
                ...
                LOGMUX(LOG_DEBUG, "Writing %d byte frame received on channel %d to %s",
                    frame->length,frame->channel, channel->ptsname);
                ...
                if ( frame != NULL )
                {
                    ...
                    while ( 1 )
                    {
                        ...
                        {
                            LOGMUX( LOG_DEBUG, "write() returned. Written %d/%d bytes of frame to %s",
	...
	Serial* serial )
{
    ...
    if (!strcmp(serial->devicename, "/dev/socket/qemud")) {
        ...
        SYSCHECK(serial->fd = socket_local_client( "qemud",
    ...
    {   
        ...
        if (serial->fd <0) {
            ...
            SYSCHECK(serial->fd = socket_local_client( serial->devicename,
    ...
    Serial * serial)
{
    ...
    switch (serial->state)
    {
        ...
        case MUX_STATE_INITILIZING:
            ...
            if (rild_started == 0)
            {
                /* In this case, serial->state has been switched to MUX_STATE_MUXING 
                          when returning from start_muxer() successfully */                          
                rild_started++;
...
int main(int argc,char *argv[])
{
    ...
    while((main_exit_signal==0) && ((rc=watchdog(&serial))==0))
    {
        ...
        LOGMUX(LOG_INFO, "Frames received/dropped: %lu/%lu", serial.in_buf->received_count, 
        ...
        {
            char prop_value[2]={0};
            property_get("mux.force.assert.flag",prop_value,NULL);                
            if (prop_value[0] == 'Y')
            {
