...
static WLAN_REQ_ENTRY arWlanOidReqTable[] = {
    ...
    */
    ...
    { OID_802_11_RSSI,
        ...
        TRUE, FALSE, ENUM_OID_DRIVER_CORE, sizeof(PARAM_RSSI),
    ...
    { OID_802_11_DESIRED_RATES,
        DISP_STRING("OID_802_11_DESIRED_RATES"),
        FALSE, FALSE, ENUM_OID_DRIVER_CORE, sizeof(PARAM_RATES),
        (PFN_OID_HANDLER_FUNC_REQ)wlanoidQueryDesiredRates,
        (PFN_OID_HANDLER_FUNC_REQ)wlanoidSetDesiredRates },        
...
/* Renbang : ++ (20100319)*/        
#if 1
   { OID_CUSTOM_FIXED_RX_GAIN,
       DISP_STRING("OID_CUSTOM_FIXED_RX_GAIN"),
       FALSE, FALSE, ENUM_OID_DRIVER_CORE, 4,
       NULL,
       (PFN_OID_HANDLER_FUNC_REQ)wlanoidSetFixedRxGain },
#endif       
/* Renbang : -- (20100319)*/

/* Renbang : ++ (20100319)*/
#if 0
    { OID_CUSTOM_MTK_EXT_CONFIG,
        DISP_STRING("OID_CUSTOM_MTK_EXT_CONFIG"),
        FALSE, FALSE, ENUM_OID_GLUE_ONLY, CFG_FILE_WIFI_REC_SIZE,
        (PFN_OID_HANDLER_FUNC_REQ)reqQueryExtConfig,
        NULL },
#endif        
...
/* Renbang : ++ (20100319)*/
    ...
    { OID_CUSTOM_PREFER_ANT,
        DISP_STRING("OID_CUSTOM_PREFER_ANT"),
        FALSE, FALSE, ENUM_OID_DRIVER_CORE, 4, 
        (PFN_OID_HANDLER_FUNC_REQ)wlanoidQueryPreferredAnt,
        (PFN_OID_HANDLER_FUNC_REQ)wlanoidSetPreferredAnt },    
#endif
/* Renbang : -- (20100319)*/
...
/* Renbang ++ 20100319 */
#if 0
    { OID_CUSTOM_WAPI_CAPABILITY,
        DISP_STRING("OID_CUSTOM_WAPI_CAPABILITY"),
        TRUE, FALSE, ENUM_OID_DRIVER_CORE, 4 * 8,
        (PFN_OID_HANDLER_FUNC_REQ)wlanoidQueryWapiCap,
        NULL },
#endif
/* Renbang -- 20100319 */        
    ...
    { OID_CUSTOM_WMMPS_MODE,
        DISP_STRING("OID_CUSTOM_WMMPS_MODE"),
        TRUE, TRUE, ENUM_OID_GLUE_ONLY, sizeof(UINT_8),
        (PFN_OID_HANDLER_FUNC_REQ)wlanoidQueryWmmPsMode,
        (PFN_OID_HANDLER_FUNC_REQ)wlanoidSetWmmPsMode },
};
    ...
    )
{
    ...
    case PRIV_CMD_VOIP_DELAY:
        /* TODO(Kevin 2008/03/31): Currently we only set a UINT_32 voip interval, so
         * we can use the priv_set_int now. If we add more paramters to PARAM_VOIP_CONFIG,
         * we should use another IOCTL.
         */
        rStatus = wlanSetInformation(prGlueInfo->prAdapter,
    ...
    case PRIV_CUSTOM_CONTINUOUS_POLL:
        rStatus = wlanSetInformation(prGlueInfo->prAdapter,
                           ...
                           &u4BufLen);
        break;
    case PRIV_CUSTOM_DISABLE_BEACON_DETECTION:
        rStatus = wlanSetInformation(prGlueInfo->prAdapter,
    ...
    )
{
    ...
    if (!prNetDev || !prIwReqData) {
        printk(KERN_INFO DRV_NAME "priv_get_struct(): invalid param(0x%p, 0x%p)\n",
    ...
    ASSERT(prGlueInfo);
    if (!prGlueInfo) {
        printk(KERN_INFO DRV_NAME "priv_get_struct(): invalid prGlueInfo(0x%p, 0x%p)\n",
    ...
    case PRIV_CMD_OID:
        if (copy_from_user(&aucOidBuf[0],
            ...
            printk(KERN_NOTICE "priv_get_struct() copy_from_user oidBuf fail\n");
            return -EFAULT;
        }
        ...
        if (priv_get_ndis(prNetDev, prNdisReq, &u4BufLen) == 0) {
            ...
            if (copy_to_user(prIwReqData->data.pointer,
                ...
                printk(KERN_NOTICE "priv_get_struct() copy_to_user oidBuf fail(1)\n");
                return -EFAULT;
            }
            return 0;
        }
            ...
            if (copy_to_user(prIwReqData->data.pointer,
                    &aucOidBuf[0],
                    OFFSET_OF(NDIS_TRANSPORT_STRUCT, ndisOidContent))) {
                printk(KERN_NOTICE "priv_get_struct() copy_to_user oidBuf fail(2)\n");
            }
            return -EFAULT;
        }
        break;
    ...
    default:
        printk(KERN_WARNING "get struct cmd:0x%lx\n", u4SubCmd);
        return -EOPNOTSUPP;
    }
} /* priv_get_struct */
    ...
    )
{
    ...
    if (!prNetDev || !prNdisReq || !pu4OutputLen) {
        printk(KERN_INFO DRV_NAME "priv_set_ndis(): invalid param(0x%p, 0x%p, 0x%p)\n",
    ...
    ASSERT(prGlueInfo);
    if (!prGlueInfo) {
        printk(KERN_INFO DRV_NAME "priv_set_ndis(): invalid prGlueInfo(0x%p, 0x%p)\n",
            prNetDev, netdev_priv(prNetDev));
        return -EINVAL;
    }
