...
#define SYS_OCP_REGS_SYS_PHYS_BASE		(SYS_SGX_REGS_SYS_PHYS_BASE + EUR_CR_OCP_REVISION)
#define SYS_OCP_REGS_SIZE				0x110
...
static void SGXEarlySuspendHandler(struct early_suspend *h)
{
	printk("PVR: early suspending...\n");
	DisableSGXClocks_t(gpsSysData);
}

static void SGXEarlyResumeHandler(struct early_suspend *h)
{
	printk("PVR: resuming...\n");
	EnableSGXClocks_t(gpsSysData);
}

static struct early_suspend sgx_early_suspend_handler =
{
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 50,
	.suspend = SGXEarlySuspendHandler,
	.resume = SGXEarlyResumeHandler,
};
...
PVRSRV_ERROR SysInitialise(IMG_VOID)
{
...
#if defined(SGX_OCP_REGS_ENABLED)
	{
		...
		sOCPRegsSysPBase.uiAddr	= SYS_OCP_REGS_SYS_PHYS_BASE;
	...
	register_early_suspend(&sgx_early_suspend_handler);
#endif	
