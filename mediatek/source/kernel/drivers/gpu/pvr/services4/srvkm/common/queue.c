													 ...
													 PVRSRV_QUEUE_INFO **ppsQueueInfo)
{
	...
	if (eError != PVRSRV_OK)
	{
		...
		printk(KERN_ERR "PVR_K: PVRSRVCreateCommandQueueKM: Failed to alloc queue struct\n");
		goto ErrorExit;
	}
	...
	if (eError != PVRSRV_OK)
	{
		...
		printk(KERN_ERR "PVR_K: PVRSRVCreateCommandQueueKM: Failed to alloc queue buffer\n");
		goto ErrorExit;
	}
	...
	if (psSysData->psQueueList == IMG_NULL)
	{
		...
		if (eError != PVRSRV_OK)
		{
			printk(KERN_ERR "PVR_K: PVRSRVCreateCommandQueueKM: Failed to create resource\n");
			goto ErrorExit;
		}
	}
	...
	if (eError != PVRSRV_OK)
	{
		printk(KERN_ERR "PVR_K: PVRSRVCreateCommandQueueKM: Failed to lock resource\n");
		goto ErrorExit;
	}
	...
	if (eError != PVRSRV_OK)
	{
		printk(KERN_ERR "PVRSRVCreateCommandQueueKM: Failed to unlock resource\n");
		goto ErrorExit;
	}
												...
												IMG_VOID **ppvSpace)
{
	...
	if (ui32ParamSize > PVRSRV_MAX_CMD_SIZE)
	{
		...
		printk(KERN_WARNING "PVR_K: PVRSRVGetQueueSpace: max command size is %d bytes\n", PVRSRV_MAX_CMD_SIZE);
	...
	if (bTimeout == IMG_TRUE)
	{
		*ppvSpace = IMG_NULL;
		...
		printk(KERN_WARNING "PVR_K: PVRSRVGetQueueSpaceKM: Timeout...\n");
												...
												IMG_SIZE_T			ui32DataByteSize )
{
	...
	if(eError != PVRSRV_OK)
	{
		printk(KERN_ERR "PVR_K: PVRSRVInsertCommandKM: Fail to get queue space\n");
		return eError;
	}
								  ...
								  IMG_BOOL			bFlush)
{
	...
	while (psWalkerObj < psEndObj)
	{
		...
		if (!psSyncData)
		{
			printk(KERN_ERR "PVR_K: PVRSRVProcessCommand(dst): Deleted psWalkerObj(0x%p)->psKernelSyncInfoKM(0x%p)->psSyncData(0x%p)\n", psWalkerObj, psWalkerObj->psKernelSyncInfoKM, psWalkerObj->psKernelSyncInfoKM->psSyncData);
			psWalkerObj++;
			continue;
		}
	...
	while (psWalkerObj < psEndObj)
	{
		...
		if (!psSyncData)
		{
			printk(KERN_ERR "PVR_K: PVRSRVProcessCommand(src): Deleted psWalkerObj(0x%p)->psKernelSyncInfoKM(0x%p)->psSyncData(0x%p)\n", psWalkerObj, psWalkerObj->psKernelSyncInfoKM, psWalkerObj->psKernelSyncInfoKM->psSyncData);
			psWalkerObj++;
			continue;
		}
