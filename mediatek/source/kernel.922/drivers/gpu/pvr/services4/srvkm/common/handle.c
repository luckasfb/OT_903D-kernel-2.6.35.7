...
PVRSRV_ERROR PVRSRVLookupSubHandle(PVRSRV_HANDLE_BASE *psBase, IMG_PVOID *ppvData, IMG_HANDLE hHandle, PVRSRV_HANDLE_TYPE eType, IMG_HANDLE hAncestor)
{
	...
	if (eError != PVRSRV_OK)
	{
		...
		printk(KERN_ERR "PVR_K: PVRSRVLookupSubHandle: Error looking up subhandle (%d)\n", eError);
		return eError;
	}
	...
	for (psPHand = psCHand; ParentHandle(psPHand) != hAncestor; )
	{
		...
		if (eError != PVRSRV_OK)
		{
			...
			printk(KERN_ERR "PVR_K: PVRSRVLookupSubHandle: Subhandle doesn't belong to given ancestor\n");
