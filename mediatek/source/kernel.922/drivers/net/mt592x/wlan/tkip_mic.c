...
#define MASK_QOSCTL_TID               BITS(0,3)

#define MK16(x,y)       (((UINT_16) (x) << 8) | (UINT_16) (y))
...
#define ROTR32(x,y)     (((x) >> (y)) | ((x) << (32 - (y)))) // rotate-right 32-bit variable x y-bit, OK
#define ROTL32(x,y)     (((x) << (y)) | ((x) >> (32 - (y)))) // rotate-left 32-bit variable x y-bit, OK
#define ROTR16(x,y)     (((x) >> (y)) | ((x) << (16 - (y)))) // rotate-right 16-bit variable x y-bit, OK
#define ROTL16(x,y)     (((x) << (y)) | ((x) >> (16 - (y)))) // rotate-left 16-bit variable x y-bit, OK
    ...
    )
{
    ...
    pucMicKey = &prAdapter->arWlanCtrl[0].rSWKey.aucTxMicMaterial[0];

    DBGLOG(RSN, INFO, ("MIC key %02x-%02x-%02x-%02x %02x-%02x-%02x-%02x\n",
    ...
    DBGLOG(RSN, INFO, ("Mic %02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x\n",
    ...
    )
{
    ...
    ASSERT(prAdapter);
    ASSERT(prSwRfb);
    ASSERT(pu2ResultFrameBodyLen);

    *pu2ResultFrameBodyLen = prSwRfb->u2FrameLength;
    ...
    ucEntry = prSwRfb->prRxStatus->ucWlanIdx & BITS(0,3);
    if (ucEntry >= WLAN_TABLE_SIZE) {
        DBGLOG(RSN, WARN, ("Received packet can't got key to decrypting! \n"));
        return FALSE;
    }

    pucMickey = &prAdapter->arWlanCtrl[ucEntry].rSWKey.aucRxMicMaterial[0];

    DBGLOG(RSN, LOUD, ("Before TKIP MSDU Decapsulate:\n"));
    DBGLOG(RSN, LOUD, ("MIC key:\n"));
    DBGLOG_MEM8(RSN, LOUD, pucMickey, 8);

    pucDa = (PUINT_8)(&(((P_WLAN_MAC_HEADER_T)prSwRfb->pvHeader)->aucAddr1[0]));
    pucSa = (PUINT_8)(&(((P_WLAN_MAC_HEADER_T)prSwRfb->pvHeader)->aucAddr3[0]));
    ...
    case 0:
        pucDa = (PUINT_8)(&(((P_WLAN_MAC_HEADER_T)prSwRfb->pvHeader)->aucAddr1[0]));
        pucSa = (PUINT_8)(&(((P_WLAN_MAC_HEADER_T)prSwRfb->pvHeader)->aucAddr2[0]));
        break;
    ...
    if (prRxStatus->u2StatusFlag & RX_STATUS_FLAG_QoS) {
        ucPriority = (UCHAR) (prRxStatus->u2QoSCtrl & MASK_QOSCTL_TID);
    }
    else {
        ucPriority = 0;
    }
    ...
    *pu2ResultFrameBodyLen = prSwRfb->u2FrameLength - WLAN_MAC_MIC_LEN;
