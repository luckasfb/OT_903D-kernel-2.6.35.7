                               ...
                               IMG_UINT32 ui32Tries)
{
	...
	for(;;)
	{
		...
		if (psSysData->psGlobalEventObject)
		{
			...
			if(psSysData->psGlobalEventObject)
			{
				eError = OSEventObjectOpen(psSysData->psGlobalEventObject, &hOSEventKM);
				if (eError |= PVRSRV_OK)
				{
				...
				eError = OSEventObjectWait(hOSEventKM);
				if (eError != PVRSRV_OK)
				{
					PVR_DPF((PVR_DBG_ERROR,
				...
				eError = OSEventObjectClose(psSysData->psGlobalEventObject, hOSEventKM);
				if (eError != PVRSRV_OK)
				{
					PVR_DPF((PVR_DBG_ERROR,
													   ...
													   IMG_HANDLE				*phOSMapInfo)
{
	...
	if (!bBMError)
	{
		...
		printk(KERN_ERR "PVR_K: PVRSRVMapDeviceClassMemoryKM: BM_Wrap Failed\n");
