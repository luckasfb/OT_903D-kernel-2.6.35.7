...
void FT_GetVersionInfo(FT_VER_INFO_REQ *req, char *pft_PeerBuf, kal_int16 ft_peer_len)
{
	...
	META_LOG("[FTT_Drv:] FT_GetVersionInfo ");

    if((fd = fopen(VERSION_FILE_PATH,"r"))==NULL)
    {
        META_LOG("FT_GetVersionInfo Can't open file : %s\n", VERSION_FILE_PATH);
    }
	...
	{
		META_LOG("[FTT_Drv:] String is too long, length=%d ", strlen(szBuffer));
	}
    fclose(fd);

	/* Get Software version : ft_cnf.sw_ver */
	META_LOG("[FTT_Drv:] ft_cnf.sw_ver = %s ", ft_cnf.sw_ver);

	/* Get the build time : ft_cnf.sw_ver */
	META_LOG("[FTT_Drv:] ft_cnf.sw_time = %s ", ft_cnf.sw_time);

	/* Get the chip version : ft_cnf.sw_ver */
	META_LOG("[FTT_Drv:] ft_cnf.bb_chip = %s ", ft_cnf.bb_chip);
	META_LOG("[FTT_Drv:] ft_cnf.hw_ver = %s ", ft_cnf.hw_ver);
...
void FT_CPURegW_OP(FT_REG_WRITE_REQ *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    META_LOG("[FTT_Drv:] FT_CPURegW_OP META Test ");
...
void FT_CPURegR_OP(FT_REG_READ_REQ *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    META_LOG("[FTT_Drv:] FT_CPURegR_OP META Test: %d ", sizeof(FT_REG_READ_CNF) );
...
void FT_PMICRegR_OP(FT_PMIC_REG_READ *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    META_LOG("[FTT_Drv:] FT_PMICRegR_OP META Test ");
...
void FT_PMICRegW_OP(FT_PMIC_REG_WRITE *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    META_LOG("[FTT_Drv:] FT_PMICRegW_OP META Test ");
...
void FT_Peripheral_OP(FT_UTILITY_COMMAND_REQ *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    META_LOG("[FTT_Drv:] FT_Peripheral_OP META Test ");
    ...
    if (bInitFlag_Peri == FALSE)
    {
        if (!Meta_LCDBK_Init())
        {
            META_LOG("[FTT_Drv:] FT_Peripheral_OP Meta_LCDBK_Init Fail ");
            goto Per_Exit;
        }
        /*if (!Meta_NledInit())
        {
            META_LOG("[FTT_Drv:] FT_Peripheral_OP Meta_NledInit Fail ");
            goto Per_Exit;
        }*/
        if (!Meta_KeypadBK_Init())
        {
            META_LOG("[FTT_Drv:] FT_Peripheral_OP Meta_KeypadBK_Init Fail ");
            goto Per_Exit;
        }
         if (!Meta_LCDFt_Init())
        {
            META_LOG("[FTT_Drv:] FT_Peripheral_OP Meta_LCDFt_Init Fail ");
            goto Per_Exit;
        }
        if (!Meta_Vibrator_Init())
        {
            META_LOG("[FTT_Drv:] FT_Peripheral_OP Meta_Vibrator_Init Fail ");
            goto Per_Exit;
        }
        bInitFlag_Peri =  TRUE;
    ...
    case FT_UTILCMD_KEYPAD_LED_ONOFF:			//test keypad from meta keypad lib
        UtilityCnf.result.m_KeypadBKCnf =Meta_KeypadBK_OP(pFTReq->cmd.m_KeypadBKReq);
    ...
    case FT_UTILCMD_SET_CLEAN_BOOT_FLAG:		
		META_LOG("[FTT_Drv:] FT_UTILCMD_SET_CLEAN_BOOT_FLAG META Test %s,%d,%s",__FILE__,__LINE__,__FUNCTION__);
...
{
    META_LOG("[FTT_Drv:] FT_APEditorRead_OP META Test ");
    ...
    if (!META_Editor_ReadFile_OP(pFTReq))
        META_LOG("[FTT_Drv:] FT_APEditorR_OP META Test Fail");
}
...
void FT_APEditorWrite_OP(FT_AP_Editor_write_req *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    META_LOG("[FTT_Drv:] FT_APEditorW_OP META Test ");
...
void FT_APEditorReset_OP(FT_AP_Editor_reset_req *pFTReq)
{
    ...
    META_LOG("[FTT_Drv:] FT_APEditorReset_OP META Test ");
...
void FT_BAT_OP(FT_BATT_REQ *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    if (FALSE == bInitFlag_BAT)
    {
        ...
        if (!Meta_Battery_Init())
        {
            ...
            META_LOG("[FTT_Drv:] FT_BAT_OP Meta_Battery_Init Fail ");
            goto  BAT_Exit;
        }
        bInitFlag_BAT = TRUE;
    }
...
ADC_Exit:
    ...
    META_LOG("[FTT_Drv:] FT_AUXADC_OP Test req: %d , %d ", sizeof(AUXADC_REQ), sizeof(AUXADC_CNF));
    WriteDataToPC(&ft_cnf, sizeof(ft_cnf),NULL, 0);
}
...
WIFI_Exit:
    META_LOG("[FTT_Drv:] FT_WIFI_OP Test Fail: %d , %d ", sizeof(FT_WM_WIFI_REQ), sizeof(FT_WM_WIFI_CNF));
    WriteDataToPC(&ft_cnf, sizeof(ft_cnf),NULL, 0);
}
...
{
	META_LOG("[FTT_Drv:] FT_GPS_OP Test %d , %d ", sizeof(GPS_REQ), sizeof(GPS_CNF));
...
void FT_L4AUDIO_OP(FT_L4AUD_REQ *FTReq, char *pPeerBuf, kal_int16 peer_len)
{
	...
	if(FALSE == bInitFlag_L4AUD)
	{
		if(!META_Audio_init())
		{
			...
			META_LOG("[FTT_Drv:] FT_L4AUD_OP META_Audio_init Fail ");
			goto  L4AUDIO_Exit;
		}
...
void FT_SDcard_OP(SDCARD_REQ *req, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    META_LOG("[FTT_Drv:] FT_SDcard_OP META Test req: %d , %d ",
...
SDcard_Exit:
    META_LOG("[FTT_Drv:] FT_SDcard_OP Meta_SDcard_Init Fail ");
...
void FT_LOW_POWER_OP(FT_LOW_POWER_REQ *req, char *pPeerBuf, kal_int16 peer_len)
{
   ...
   FT_LOW_POWER_CNF ft_cnf;
    ...
    META_LOG("[FTT_Drv:] FT_LOW_POWER_OP META Test ");
   ...
   if((FTReq->op != FT_CCT_OP_SUBPREVIEW_LCD_START) && (FTReq->op !=FT_CCT_OP_SUBPREVIEW_LCD_STOP))
   	{
   		...
   		if(!META_CCAP_init())
   		{
   				...
   				ft_cnf.op = FTReq->op;
   				ft_cnf.status = META_FAILED;
   				META_LOG("[FTT_Drv:] FT_CCAP_OP META_CCT_init Fail ");
   				goto CCT_Exit;
   		}
...
void FT_GPIO_OP(GPIO_REQ *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    if (FALSE == bInitFlag_GPIO)
    {
        ...
        if (!Meta_GPIO_Init())
        {
            ...
            META_LOG("[FTT_Drv:] FT_GPIO_OP Meta_GPIO_Init Fail ");
            goto  GPIO_Exit;
        }
        bInitFlag_GPIO = TRUE;
    }
...
const char* makepath(unsigned char file_ID)
{
	...
	else
	{
		...
		{
			META_LOG("[FTT_Drv:] makepath error: invalid file_ID %d! ", file_ID);
			return "";
		}
	}
}
...
bool SendNVRAMFile(unsigned char file_ID, FT_NVRAM_BACKUP_CNF* pft_cnf)
{
	...
	if(backupFd >= 0)
	{
		META_LOG("[FTT_Drv:] File%d opens succeed ! ",file_ID);
		...
		while(!(pft_cnf->block.stage & BLK_EOF))
		{
			...
			if(peer_buff_size != -1)
			{
				...
				if(peer_buff_size == 0)
				{
					...
					META_LOG("[FTT_Drv:] File%d backups succeed! ",file_ID);
					...
					{
						META_LOG("[FTT_Drv:] File%d DeleteFile succeed! ",file_ID);
						return_value = TRUE;
				...
				else
				{
					...
					META_LOG("[FTT_Drv:] File%d backups %d data ! ",file_ID,peer_buff_size);
			...
			{
				pft_cnf->block.stage |= BLK_EOF;
				META_LOG("[FTT_Drv:] File%d backups read failed ! ", file_ID);
			}
	...
	{
		META_LOG("[FTT_Drv:] File%d backups open failed ! ", file_ID);
	}
...
{
	META_LOG("[FTT_Drv:] FT_NVRAM_Backup_OP ");
	...
	if (req->count > 0)
	{
		META_LOG("[FTT_Drv:] Count is %d, backup parts of NvRam!", req->count);
		bFileOpResult = FileOp_BackupData_Special(req->buffer, req->count, req->mode);
	}
	else
	{
		META_LOG("[FTT_Drv:] Count is %d, backup all NvRam!", req->count);
		bFileOpResult = FileOp_BackupAll_NvRam();
	}
	
	if(bFileOpResult)
	{
	    META_LOG("[FTT_Drv:] NVM_PcBackup_Get_Data Start ! ");
		if(SendNVRAMFile(0,&ft_cnf))
		{
			META_LOG("[FTT_Drv:] Send file 0 succeed! ! ");
			//init the header
			ft_cnf.header.id = req->header.id +1;
			ft_cnf.header.token = req->header.token;
			ft_cnf.status = META_FAILED;
			...
			{
				META_LOG("[FTT_Drv:] Send file 1 succeed! ! ");
				return;
			}
		}
	}
	...
	{
		META_LOG("[FTT_Drv:] Failed to backup NvRam!");
	}
...
void FT_NVRAM_Restore_OP(FT_NVRAM_RESTORE_REQ * req, char* pPeerBuf, kal_int16 peer_len)
{
	...
	META_LOG("[FTT_Drv:] FT_NVRAM_Restore_OP receive block stage %x  file id %d file size %d!",req->block.stage,req->block.file_ID,req->block.file_size);
	...
	if(backupFd >= 0)
	{
		META_LOG("[FTT_Drv:] FT_NVRAM_Restore_OP create or open file OK!");
		kal_uint16 sWriten = 0;
		sWriten = write(backupFd,pPeerBuf,peer_len);
		if(sWriten)
		{
			ft_cnf.status = META_SUCCESS;
			META_LOG("[FTT_Drv:] FT_NVRAM_Restore_OP File%d write %d data total data %d!",req->block.file_ID,sWriten,peer_len);
			if(req->block.stage & BLK_EOF)
			{
				fileLen = getFileSize(backupFd);
				if(req->block.file_size == fileLen)
				{
					META_LOG("[FTT_Drv:] FT_NVRAM_Restore_OP write file transfer success! ");
					close(backupFd);
					backupFd = -1;
					...
					if(req->block.file_ID == 1)
					{
						...
						{
							ft_cnf.status = META_FAILED;
							META_LOG("[FTT_Drv:] META_Editor_PcRestore_Set_Data failed! ");
				...
				{
					ft_cnf.status = META_FAILED;
					META_LOG("[FTT_Drv:] FT_NVRAM_Restore_OP file %d size error! / %d ",req->block.file_ID,req->block.file_size);
				}
			}
		}
		else
		{
			META_LOG("[FTT_Drv:] FT_NVRAM_Restore_OP write file failed!");
		}
	...
	{
		META_LOG("[FTT_Drv:] FT_NVRAM_Restore_OP create or open file failed!");
	}
...
{
	META_LOG("[FTT_Drv:] FT_GSENSOR_OP");
    GS_CNF ft_cnf;
    ...
    if (FALSE == bInitFlag_GS)
    {
        ...
        if (!Meta_GSensor_Open())
        {
            ...
            META_LOG("[FTT_Drv:] FT_GSENSOR_OP Meta_GSensor_Open Fail ");
            goto  GS_Exit;
        }
        bInitFlag_GS = TRUE;
    }
...
{
	META_LOG("[FTT_Drv:] FT_GYROSENSOR_OP");
    GYRO_CNF ft_cnf;
    ...
    if (FALSE == bInitFlag_GYRO)
    {
        ...
        if (!Meta_Gyroscope_Open())
        {
            ...
            META_LOG("[FTT_Drv:] FT_GYROSENSOR_OP Meta_GYROSensor_Open Fail ");
            goto  GYRO_Exit;
        }
        bInitFlag_GYRO = TRUE;
    }
...
void FT_META_MODE_LOCK(FT_META_MODE_LOCK_REQ *pFTReq)
{
    ...
    META_LOG("[FTT_Drv:] FT_META_MODE_LOCK ");
    ...
    META_LOG("[FTT_Drv:] FT_META_MODE_LOCK Status is %d", lock_status);
...
{
    META_LOG("[FTT_Drv:] FT_Reboot: Device will reboot after %d seconds. ", pFTReq->delay);
	sleep(pFTReq->delay);
...
{
    META_LOG("[FTT_Drv:] FT_CUSTOMER_OP!");
	...
	META_LOG("[FTT_Drv:] FT_CUSTOMER_OP successful, OP type is %d!", pFTReq->type);
...
{
    META_LOG("[FTT_Drv:] FT_GET_CHIPID_OP!");
	...
	if (fd != -1)
	{
		...
		close(fd);
    	ft_cnf.status = META_SUCCESS;
		META_LOG("Chip rid=%s", ft_cnf.chipId);
	}
	else
	{
		if (errno == ENOENT)
		...
		META_LOG("Failed to open chip rid file %s, errno=%d", CHIP_RID_PATH, errno);
	}
...
{
	META_LOG("[FTT_Drv:] FT_MSENSOR_OP");
    ...
    if (FALSE == bInitFlag_MS)
    {
        ...
        {
            META_LOG("[FTT_Drv:] FT_MSENSOR_OP Meta_MSensor_Open failed!");
			ft_cnf.status = META_FAILED;
            goto  MS_Exit;
        }
        bInitFlag_MS = TRUE;
...
void FT_ALSPS_OP(FT_ALSPS_REQ *pFTReq, char *pPeerBuf, kal_int16 peer_len)
{
    ...
    if (FALSE == bInitFlag_ALSPS)
    {
        ...
        if (!Meta_ALSPS_Open())
        {
            META_LOG("[FTT_Drv:] FT_ALSPS_OP Meta_ALSPS_Open failed!");
			ft_cnf.status = META_FAILED;
            goto  MS_Exit;
        }
...
void FT_GetVersionInfoV2(FT_VER_INFO_V2_REQ *req, char *pft_PeerBuf, kal_int16 ft_peer_len)
{
    ...
    while(!feof(fd))
    {
        if(fgets(str, 256, fd)!=NULL)
        {
            ...
            {
                META_LOG("[FT_GetVersionInfo] Build Display ID = %s\n", tmp);
	...
	if (strlen(tmp) <= 64)
	{
		strcpy((char*)ft_cnf.bb_chip, tmp);
	}
	else
	{
		META_LOG("[FTT_Drv:] String is too long, length=%d ", strlen(tmp));
	}
    fclose(fd);
...
{	
	...
	META_LOG("[FTT_Drv:] FT_BUILD_PROP_OP ");
