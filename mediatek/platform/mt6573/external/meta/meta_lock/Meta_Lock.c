...
static int META_Lock_WriteFlash(unsigned int iMagicNum)
{
      ...
      char cMtdDevName[32];
      int iLogo_Partition=get_partition_numb("logo");
      META_LOG("[META_LOCK]:Logo_Partition:%d\n",iLogo_Partition);
      sprintf(cMtdDevName,"/dev/mtd/mtd%d",iLogo_Partition);
      META_LOG("[META_LOCK]:Logo_Partition:%s\n",cMtdDevName);
