...
#define FM_META_BAND FM_BAND_UE
                ...
                if(RDS_Struct.event_status&event_mask){ 
                    ...
                    switch (event_mask){
                        ...
                        case RDS_EVENT_AF:{ 
							...
							memcpy(RDS_Cnf.m_rRDS.m_rRDSInfo.m_buffer, &RDS_Struct.AF_Data.AF[0][0], sizeof(RDS_Struct.AF_Data.AF));
                            WriteDataToPC(pFM_CNF, sizeof(FM_CNF), &RDS_Cnf, sizeof(FM_RDS_CNF_T));
                        }
                            break;
...
void META_FM_OP(FM_REQ *req, char *peer_buff, unsigned short peer_len) 
{
    ...
    switch(req->op){
		...
		case FM_OP_POWER_ON:
            ...
            parm_tune.freq = FM_FREQ_MIN; //default value for FM power up.
        ...
        case FM_OP_GET_SIGNAL_VAL:
            ...
            fm_cnf.fm_result.m_rSignalValue.m_ucSignalLevel = (unsigned char)rssi;
     		fm_cnf.status = META_SUCCESS;
			WriteDataToPC(&fm_cnf, sizeof(FM_CNF), NULL, 0);
			break;
	    ...
	    */
		...
		case FM_OP_SEARCH_NEXT_STAT:
            ...
            parm_seek.space = FM_SPACE_100K; //default 100K.    
		...
		case FM_OP_FM_AUTOSCAN:                        
            parm_scan.band = FM_META_BAND;
            parm_scan.space = FM_SPACE_100K;
            ...
            if (ret){
                ...
                pScanTBL = (FM_AutoScan_CNF_T *)malloc(sizeof(FM_AutoScan_CNF_T));
                
                if(parm_scan.ScanTBLSize*sizeof(unsigned short) <= sizeof(FM_AutoScan_CNF_T))
                    memcpy((char *)(pScanTBL->m_u2Bitmap), (char *)parm_scan.ScanTBL, parm_scan.ScanTBLSize*sizeof(unsigned short));
                else
                    memcpy((char *)(pScanTBL->m_u2Bitmap), (char *)parm_scan.ScanTBL, sizeof(FM_AutoScan_CNF_T));
	        }

	        fm_cnf.status = META_SUCCESS;
	        WriteDataToPC(&fm_cnf, sizeof(FM_CNF), pScanTBL->m_u2Bitmap, sizeof(FM_AutoScan_CNF_T));       
			break;
	   ...
	   case FM_OP_SET_DYNAMIC_LIMITER:
        ...
        case FM_OP_RESET_BLOCK_COUNTER:{
			//dummy first
			DBG("FM_OP_RESET_BLOCK_COUNTER:NR\n");
