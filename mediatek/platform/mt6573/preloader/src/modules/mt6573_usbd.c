...
*  USB DEBUG
**************************************************************************/
#define  MT6573_USB_DBG_LOG   0

#if MT6573_USB_DBG_LOG
#define USB_LOG    dbg_print
#define USB_BUG() do { \
...
static struct mt_ep *mt_find_ep (int ep_num);
static void mt_udc_flush_fifo (u8 ep_num);
...
extern void mt6573_usb_phy_poweron (void);
extern void mt6573_usb_phy_savecurrent (void);
extern void mt6573_usb_phy_recover (void);
...
static int ep0_class_setup (struct urb *urb)
{
    ...
    switch (request->bRequest)
    {
        ...
        case CDCACM_REQ_SET_CONTROL_LINE_STATE:
            /* do nothing */
            USB_LOG ("CDC ACM Request: SET_CONTROL_LINE_STATE\n");
            break;
...
static int mt_read_fifo (struct mt_ep *endpoint)
{
    ...
    if (urb)
    {
        ...
        else
        {
            while (len > 0)
            {
                /* pangyen 20090910 - Added to improve performance */
                if (len >= 4)
                {
...
static struct mt_ep * mt_find_ep (int ep_num)
{
    ...
    {
        if ((udc_device->endpoint_array[i].
            endpoint_address & USB_EP_NUM_MASK) == ep_num)
...
static void mt_udc_reset (void)
{
    ...
    */
    ...
    {
        dbg_print ("USB: High Speed\n");
        enable_highspeed ();
    }
    else
    {
        dbg_print ("USB: Full Speed\n");
    }
...
static void mt_udc_ep0_setup (void)
{
    ...
    if (((request->bmRequestType) & USB_TYPE_MASK) == USB_TYPE_STANDARD)
    {
        ...
        if (stall)
        {
            dbg_print ("[USB] NOT SUPPORTED\n");
            //dbg_print ("       bmRequestType = %x\n",request->bmRequestType);
            //dbg_print ("       bRequest = %x\n", request->bRequest);
            //dbg_print ("       wValue = %x\n", request->wValue);
            //dbg_print ("       wIndex = %x\n", request->wIndex);
            //dbg_print ("       wLength = %x\n\n", request->wLength);
        }
    ...
    else if (((request->bmRequestType) & USB_TYPE_MASK) == USB_TYPE_VENDOR)
    {
        ...
        /* do nothing now */
        dbg_print ("[USB] ALL VENDOR-SPECIFIC REQUESTS ARE NOT SUPPORTED!!\n");
    }
...
void mt6573_udc_irq (u8 intrtx, u8 intrrx, u8 intrusb)
{
...
static void udc_stall_ep (unsigned int ep_num)
{
    struct mt_ep *endpoint = mt_find_ep (ep_num);
    u8 index;
    u16 csr;
...
bool mt_usb_connected(void)
{
    return (__raw_readb(POWER) & PWR_SOFT_CONN);
}
...
{
    struct mt_ep *ep;
    dbg_print ("usb phy recover\n");
    mt6573_usb_phy_recover ();
