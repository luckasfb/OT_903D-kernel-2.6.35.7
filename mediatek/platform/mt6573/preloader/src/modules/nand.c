...
#define WAIT_NFI_PIO_READY(timeout) \
    ...
    if(timeout == 0)\
   {\
   MSG(ERR, "Error: PIO_READY timeout at line=%d\n", __LINE__);\
   }\
    } while(0);
...
/*
struct nand_oobinfo mt6573_nand_oob = {
...
*  reset descriptor
**************************************************************************/
void mt6573_nand_reset_descriptor (void)
{
...
//---------------------------------------------------------------------------
static bool mt6573_nand_set_address (u32 u4ColAddr, u32 u4RowAddr, u16 u2ColNOB,
                         u16 u2RowNOB)
{
    /* fill cycle addr */
...
*
******************************************************************************/
static bool mt6573_nand_check_bch_error (u8 * pDataBuf, u32 u4SecIndex, u32 u4PageAddr)
{
    bool bRet = TRUE;
    ...
    if (0 != (u4ErrorNumDebug & 0xFFFF))
    {
        for (i = 0; i <= u4SecIndex; ++i)
        {
            ...
            if (0xF == u4ErrNum)
            {
                ...
                bRet = FALSE;
                MSG (INIT, "UnCorr at Page=%d, Sector=%d\n",
                    u4PageAddr, i);
            }
            else
            {
                //mtd->ecc_stats.corrected++;
                MSG (INIT, "Corr at Page=%d, Sector=%d\n",
...
static bool mt6573_nand_read_status(void)
{
    int status, i;
    mt6573_nand_reset();
...
static void mt6573_nand_configure_lock (void)
{
    ...
    // Workaround method for ECO1 mt6573
...
//---------------------------------------------------------------------------
static void mt6573_nand_command_bp (unsigned command)
{
    u32 timeout;
    ...
    case NAND_CMD_READID:
        ...
        mt6573_nand_reset ();
        mt6573_nand_set_mode (CNFG_OP_SRD);
        mt6573_nand_set_command (NAND_CMD_READID);
        mt6573_nand_set_address (0, 0, 1, 0);
...
//-----------------------------------------------------------------------------
static u8 mt6573_nand_read_byte (void)
{
...
bool getflashid(u16 *id)
{
	...
	mt6573_nand_command_bp (NAND_CMD_READID);

	maf_id = mt6573_nand_read_byte ();
	...
	//*id= (dev_id<<8)|maf_id;
        *id= (maf_id<<8)|dev_id;
#ifdef DBG_PRELOADER	
	MSG (INIT, "getflashID %x successful!!!\n",*id);
#endif
	return TRUE;
}

int mt6573_nand_init (void)
{
    int i, j, busw;
    u16 id;
    u32 ext_id;
    ...
    nand_maf_id = mt6573_nand_read_byte ();
    nand_dev_id = mt6573_nand_read_byte ();
    ...
    /*MSG(INIT, "%x %x %dKB %d %d %x %x %dB %dMB\n", devinfo->id, devinfo->ext_id, devinfo->blocksize,
                    devinfo->addr_cycle,
                    devinfo->iowidth,
                    devinfo->timmingsetting,
                    devinfo->advancedmode, devinfo->pagesize, devinfo->totalsize);
 */
...
//-----------------------------------------------------------------------------
static bool mt6573_nand_read_page_data (u32 * buf)
{
    u32 timeout = 0xFFFF;
...
//-----------------------------------------------------------------------------
static bool mt6573_nand_write_page_data (u32 * buf)
{
    u32 timeout = 0xFFFF;
    u32 u4Size = g_nand_chip.oobblock;
...
//-----------------------------------------------------------------------------
static void mt6573_nand_read_fdm_data (u32 u4SecNum, u8 * spare_buf)
{
    u32 i;
...
//-----------------------------------------------------------------------------
static void
mt6573_nand_write_fdm_data (u32 u4SecNum, u8 *oob)
{
    u32 i;
    u32 *pBuf32 = (u32 *)oob;
...
//---------------------------------------------------------------------------
static bool mt6573_nand_ready_for_read (u32 page_addr, u32 sec_num, u8 *buf)
{
    ...
    if (!mt6573_nand_set_address (0, u4RowAddr, colnob, rownob))
    {
        goto cleanup;
    }
...
//-----------------------------------------------------------------------------
static bool mt6573_nand_ready_for_write (u32 page_addr, u32 sec_num, u8 *buf)
{
    bool bRet = FALSE;
    u32 u4RowAddr = page_addr;
    u32 colnob = 2;
...
//#
//#############################################################################
int mt6573_nand_read_page_hwecc (unsigned int logical_addr, char *buf)
{
    ...
    if (!mt6573_nand_read_page_hw(page_in_block + mapped_block * NAND_BLOCK_BLKS, buf, g_nand_spare)) // g_nand_spare
        return FALSE;
...
int mt6573_nand_read_page_hw (u32 page, u8 *dat, u8 *oob)
{
    bool bRet = TRUE;
    u8 *pPageBuf;
    ...
    if (mt6573_nand_ready_for_read (page, u4SecNum, pPageBuf))
    {
        if (!mt6573_nand_read_page_data ((u32 *) pPageBuf))
        {
            bRet = FALSE;
        }
        ...
        //mt6573_nand_read_fdm_data(u4SecNum, (char*)(buf + 2048));
        mt6573_nand_read_fdm_data (u4SecNum, oob);
        //mt6573_nand_read_fdm_data(u4SecNum);

        if (!mt6573_nand_check_bch_error(pPageBuf, u4SecNum - 1, page))
        {
...
#if 0
    for (i = 0; mt6573_nand_oob.oobfree[i][1]; i++)
    {
        /* Set the reserved bytes to 0xff */
        start = mt6573_nand_oob.oobfree[i][0];
        len = mt6573_nand_oob.oobfree[i][1];
...
int mt6573_nand_write_page_hwecc (unsigned int logical_addr, char *buf)
{
    ...
    if (!mt6573_nand_write_page_hw(page_in_block + mapped_block * NAND_BLOCK_BLKS, 
            buf, g_nand_spare))
    {
...
int mt6573_nand_write_page_hw(u32 page, u8 *dat, u8 *oob)
{
    bool bRet = TRUE;
    ...
    if (mt6573_nand_ready_for_write (page, u4SecNum, dat))
    {
        mt6573_nand_write_fdm_data (u4SecNum, oob);
        if (!mt6573_nand_write_page_data ((u32 *)dat))
        {
            bRet = FALSE;
        }
        if (!mt6573_nand_check_RW_count (g_nand_chip.oobblock))
        {
            bRet = FALSE;
        }
        mt6573_nand_stop_write ();
        mt6573_nand_set_command (NAND_CMD_PAGEPROG);
        mt6573_nand_status_ready(STA_NAND_BUSY);
        return mt6573_nand_read_status();
    }
    else
    {
        return FALSE;
    ...
    mt6573_nand_read_page_hwecc (logical_addr, g_nand_buf);
...
bool nand_block_bad_hw (u32 logical_addr)
{
    ...
    if (mt6573_nand_ready_for_read (page, u4SecNum, tmp))
    {
        if (!mt6573_nand_read_page_data ((u32 *) tmp))
        {
            bRet = FALSE;
        }
        ...
        mt6573_nand_read_fdm_data (u4SecNum, g_nand_spare);

        if (!mt6573_nand_check_bch_error(tmp, u4SecNum - 1, page))
        {
            // MSG(ERASE, "check bch error !\n");
            bRet = FALSE;
        }
...
/*
    if (bRet)
    {
        ...
        dbg_print ("BAD BLOCK detected at 0x%x\n", logical_addr);
...
bool mark_block_bad_hw(u32 offset)
{
    ...
    //mt6573_nand_erase(logical_addr);
    //delay(1000);
    ...
    // MSG (BAD, "Mark block 0x%x bad\n", page_addr);
    while (DRV_Reg32 (NFI_STA_REG32) & STA_NAND_BUSY);

    if (mt6573_nand_ready_for_write (page_addr, u4SecNum, buf))
    {
          mt6573_nand_write_fdm_data (u4SecNum, g_nand_spare);
          if (!mt6573_nand_write_page_data ((u32 *) & buf))
          {
                bRet = FALSE;
          }
          if (!mt6573_nand_check_RW_count (g_nand_chip.oobblock))
    ...
    mt6573_nand_read_page_hwecc (LOGICAL_ADDR (page_addr), tmp);
...
u32 nand_find_safe_block(u32 offset)
{
	u32 original_offset = offset;
        ...
        {
            dbg_print("offset 0x%x bad block. goto 0x%x\n",original_offset, offset);
        }
...
//#
//#############################################################################
int mt6573_nand_erase (u32 logical_addr)
{
    int block = logical_addr / g_nand_chip.erasesize;
    int mapped_block = get_mapping_block_index(block);

    if (!mt6573_nand_erase_hw(mapped_block * g_nand_chip.erasesize))
    {
        MSG(INIT, "erase 0x%x failed\n", mapped_block);
...
bool mt6573_nand_erase_hw (u32 offset)
{
    bool bRet = TRUE;
    ...
    mt6573_nand_set_command (NAND_CMD_ERASE1);
    mt6573_nand_set_address (0, page_addr, 0, rownob);

    mt6573_nand_set_command (NAND_CMD_ERASE2);
    if (!mt6573_nand_status_ready(STA_NAND_BUSY))
    {
        return FALSE;
    }

    if (!mt6573_nand_read_status())
    {
        return FALSE;
    }
    return bRet;
...
bool mt6573_nand_wait_for_finish (void)
{
    while (DRV_Reg32 (NFI_STA_REG32) & STA_NAND_BUSY);
    return TRUE;
}
//
device_info_t *get_storge_info(void)
{
	gdevice_info.page_size = g_nand_chip.page_size;
	gdevice_info.pktsz = g_nand_chip.page_size + g_nand_chip.oobsize;

	return &gdevice_info; 
}

u32 nand_read_data(u8 *store_buf, u32 start, u32 img_size)
{
	u32 now = 0, chksm = 0, start_block = 0, total = 0;
        u32 pagesz = 0, pktsz = 0;
        int cnt;

        u8 *buf = (u8 *) g_common_buf;

        // calculate the number of page
        pagesz = gdevice_info.page_size;
        pktsz = gdevice_info.pktsz;
        ...
        // check if the starting block is good
...
#endif
                now = nand_find_safe_block (now);
#ifndef MT6573_CFG_USB_DOWNLOAD         
                if (now % BLOCK_SIZE == 0)
                {
                        now = nand_find_safe_block (now);
                }
#endif
                //Page Read
                if(mt6573_nand_read_page_hwecc (now, buf) == FALSE)
                {
                        dbg_print ("nand read data fail\n");
                        ASSERT (0);
                }
               
#ifndef MT6573_CFG_USB_DOWNLOAD                 
                // calculate chksm of reading NAND       
...
#endif
                // copy to mem
                memcpy ((u8 *)store_buf, (u8 *) (buf), pagesz);

                // calculate chksm of content of memory
#if PART_DEBUG_LOADING_UBOOT
                mem_chksm = nand_chksum_body (mem_chksm, (char *) store_buf, pktsz);
#endif
                // increment mem destination               
                store_buf += pagesz;
        }
        return chksm;
}
...
// ==========================================================
// storge device common Interface - Write Function
// ==========================================================
#ifdef MT6573_CFG_USB_DOWNLOAD
u32 nand_write_data (u8 * buf, u32 offset)
{
        ...
        if (mt6573_nand_write_page_hwecc (offset, buf) == FALSE)
        {   
                dbg_print ("nand write data fail\n");
                ASSERT (0);
        }   
...
// ==========================================================
// storge device common Interface - Erase Function
// ==========================================================
bool nand_erase_data (u32 offset, u32 offset_limit, u32 size)
{
        ...
        {
                dbg_print ("offset must be block align (0x%x)\n", BLOCK_SIZE);
                ASSERT (0);
        }
        ...
        dbg_print ("[ERASE] image size = 0x%x\n", img_size);
        dbg_print ("[ERASE] block num = %d\n", tblksz);

        // erase nand block
        cur_offset = offset;
        while (tblksz != 0)
        {
                if (mt6573_nand_erase (cur_offset) == FALSE)
                {
                        dbg_print ("[ERASE] fail\n");
                ...
                {
                        dbg_print ("[ERASE] offset (0x%x) exceeds limit (0x%x)\n", cur_offset, offset_limit);
                        return TRUE;
                }
        }
...
// ==========================================================
// storge device common Interface - Check If Device Is Ready To Use
// ==========================================================
void nand_wait_ready (void)
{
        // wait for NAND flashing complete
        mt6573_nand_wait_for_finish ();
}

// ==========================================================
// storge device common Interface - Checksum Calculation
// ==========================================================
u32 nand_chksum_per_file (u32 nand_offset, u32 img_size)
{
        ...
        u8 *buf = (u8 *) g_common_buf;
        /* read a packet */
        chksm = nand_read_data (buf, start, img_size);
        return chksm;
}
#endif

extern part_t mt6573_parts[PART_MAX_COUNT];
void nand_device_init(void)
{
    mt6573_nand_reset_descriptor ();
    mt6573_nand_init ();
    mt6573_part_init ();
    set_bmt_reserve_region(SET_RESERVE_SET, mt6573_parts[2].startblk * g_nand_chip.page_size);
#ifdef MT6573_CFG_USB_DOWNLOAD
    g_dev_vfunc.dev_write_data = nand_write_data;
    g_dev_vfunc.dev_erase_data = nand_erase_data;
    g_dev_vfunc.dev_wait_ready = nand_wait_ready;
    g_dev_vfunc.dev_cal_chksum_body = nand_chksum_body;
    g_dev_vfunc.dev_chksum_per_file = nand_chksum_per_file;
    g_dev_vfunc.dev_find_safe_block = nand_find_safe_block;
#endif
}
