...
#if 0
#define TTYDBG(fmt,args...) dbg_print("[%s] %s %d: "fmt, __FILE__,__FUNCTION__,__LINE__,##args)
#else
#define TTYDBG(fmt,args...) do{}while(0)
#endif

#if 0
#define TTYERR(fmt,args...) dbg_print("ERROR![%s] %s %d: "fmt, __FILE__,__FUNCTION__,__LINE__,##args)
#else
          ...
          while (buf->size > 0)
            {
                ...
                if (!current_urb
                  ...
                  {
                      dbg_print ("write_buffer, no available urbs\n");
                      return;
                  }
...
//#define usbtty_event_log dbg_print
...
config_usbtty (struct mt_dev *device)
{
    ...
    for (i = 0; i <= NUM_ENDPOINTS; i++)
      {
          mt_setup_ep (mt_usb_device, i, &mt_usb_ep[i]);
      }
...
service_interrupts (void)
{
    ...
    if (intrtx | intrrx | intrusb)
      {
          mt6573_udc_irq (intrtx, intrrx, intrusb);
      }
...
int
mt6573_usbtty_getcn (int count, char *buf)
{
          ...
          if (usb_input_buffer.size > 0)
            {
                ...
                //dbg_print("usb_input_buffer.data = %s\n",usb_input_buffer.data);
...
void
mt6573_usbtty_putc (const char c, int flush)
{
...
void
mt6573_usbtty_putcn (int count, char *buf, int flush)
{
...
void
mt6573_usbtty_puts (const char *str)
{
    int len = strlen (str);
...
int
mt6573_usbtty_query_data_size (void)
{
