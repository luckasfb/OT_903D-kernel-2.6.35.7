...
//#else
#define LPAGE 4096
static char *page_buf;         // [LPAGE+128];
static char *page_readbuf;     // [LPAGE];
//#endif
...
extern pt_resident new_part[PART_MAX_COUNT];
...
void get_part_tab_from_complier(void)
{
	...
	while(mt6573_parts[index].flags!= PART_FLAG_END)
	{
    		
		memcpy(lastest_part[index].name,mt6573_parts[index].name,MAX_PARTITION_NAME_LEN);
		lastest_part[index].size = mt6573_parts[index].pgnum * PAGE_SIZE;
		lastest_part[index].offset = mt6573_parts[index].startblk * PAGE_SIZE;
...
int find_empty_page_from_top(int start_addr)
{
	...
	if(page_offset==(BLOCK_SIZE/PAGE_SIZE))
	{
		...
		if(!mt6573_nand_erase(start_addr)) 
...
bool find_mirror_pt_from_bottom(int *start_addr)
{
	...
	for(mpt_locate=(BLOCK_SIZE/PAGE_SIZE);mpt_locate>0;mpt_locate--)
	{
		...
		if(!mt6573_nand_read_page_hwecc(current_start_addr, page_readbuf))
		{
...
int load_exist_part_tab(u8 *buf)
{
    ...
    page_buf = (u8 *)PMT_DAT_BUFFER;
    page_readbuf = (u8 *)PMT_READ_BUFFER;
	...
	for(pt_locate=0;pt_locate<(BLOCK_SIZE/PAGE_SIZE);pt_locate++)
	{
		...
		memset(pmt_spare,0xFF,PT_SIG_SIZE);
		memset(g_nand_spare,0xFF,64);
		if(!mt6573_nand_read_page_hwecc(pt_cur_addr, page_readbuf))
		{
	...
	if(pt_locate==(BLOCK_SIZE/PAGE_SIZE))
	{
		...
		else
		{
			...
			mt6573_nand_read_page_hwecc(mirror_address, page_readbuf);
		}
	}
		...
		*(int *)sig_buf = MPT_SIG;
		...
		if(pageoffset!=0xFFFF)                                                      
		{
			...
			for(i=0;i<2;i++)
			{
				...
				if(!mt6573_nand_write_page_hwecc(current_addr, page_buf))
				{
				...
				else
				{
					...
					//read back verify
					if((!mt6573_nand_read_page_hwecc(current_addr, page_readbuf))||memcmp(page_buf,page_readbuf,PAGE_SIZE))
					{
