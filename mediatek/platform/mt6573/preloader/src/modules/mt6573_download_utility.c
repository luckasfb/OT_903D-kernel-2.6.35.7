...
extern u32 g_nand_flash_safe_start_addr;
...
u8 * get_img_type (DM_IMG_TYPE type)
{
    ...
    default:
        dbg_print ("'%s : image type = %d' ", MOD, type);
        return "UNKNOWN";
    }
}
...
{
    u32 i = 0;
    dbg_print ("Dump spare data :\n");
    for (i = 2049; i < 2048 + 64; i++)
    {
        dbg_print ("USB buf[%d] : 0x%x\n", i, buf[i]);
    }
}
#endif
...
**************************************************************************/
#ifndef GET_IMG_BOUNDARY_FROM_TOOL
u32 get_part_range (DM_IMG_TYPE img_type)
{
    ...
    part = mt6573_part_get_partition (name);

    DM_ASSERT (part);

    return ((part->pgnum) * PAGE_SIZE);
}
#endif // #ifndef GET_IMG_BOUNDARY_FROM_TOOL
