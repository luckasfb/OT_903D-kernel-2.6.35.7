...
#define RTC_OSC32CON_GPIOCKEN	(1U << 9)	/* ungate 32K to RTC_GPIO */
...
void rtc_read_pwron_alarm(struct rtc_wkalrm *alm)
{
	...
	alm->enabled = !!(pdn1 & 0x0080);	/* return Power-On Time bit */
	...
	tm->tm_sec = RTC_PWRON_SEC;

	if (rtc_show_alarm) {
		printk(RTC_SAY "power-on time = %02d/%02d %02d:%02d:%02d (%d)(%d)\n",
		       tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min,
		       tm->tm_sec, alm->enabled, alm->pending);
	}
}
...
{
	printk(RTC_SAY "!!! 32K WAS STOPPED !!!\n");
	printk(RTC_SAY "RTC_BBPU      = 0x%x\n", rtc_read(RTC_BBPU));
	printk(RTC_SAY "RTC_IRQ_STA   = 0x%x\n", rtc_read(RTC_IRQ_STA));
	printk(RTC_SAY "RTC_IRQ_EN    = 0x%x\n", rtc_read(RTC_IRQ_EN));
	printk(RTC_SAY "RTC_OSC32CON  = 0x%x\n", rtc_read(RTC_OSC32CON));
	printk(RTC_SAY "RTC_POWERKEY1 = 0x%x\n", rtc_read(RTC_POWERKEY1));
	printk(RTC_SAY "RTC_POWERKEY2 = 0x%x\n", rtc_read(RTC_POWERKEY2));
	printk(RTC_SAY "RTC_SPAR0     = 0x%x\n", rtc_read(RTC_SPAR0));
	printk(RTC_SAY "RTC_PROT      = 0x%x\n", rtc_read(RTC_PROT));
	printk(RTC_SAY "RTC_CON       = 0x%x\n", rtc_read(RTC_CON));
	printk(RTC_SAY "RTC_TC_SEC    = %02d\n", rtc_read(RTC_TC_SEC));
	mdelay(2000);
	printk(RTC_SAY "RTC_TC_SEC    = %02d\n", rtc_read(RTC_TC_SEC));
	BUG();
}
...
static irqreturn_t __tcmfunc rtc_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(MT6573_RTC_IRQ_LINE);
...
static void rtc_reset_to_deftime(struct rtc_time *tm)
{
	...
	printk(RTC_SAY "reset to default date %04d/%02d/%02d\n",
...
static int rtc_ops_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	printk(RTC_SAY "set tc time = %04d/%02d/%02d %02d:%02d:%02d\n",
...
static void rtc_save_pwron_time(bool enable, struct rtc_time *tm)
{
	u16 pdn1, pdn2, spar1;

	pdn2 = rtc_read(RTC_PDN2) & 0xfff0;
	pdn2 |= tm->tm_mon;
...
static int rtc_pdrv_probe(struct platform_device *pdev)
{
	...
	r = request_irq(MT6573_RTC_IRQ_LINE, (irq_handler_t)rtc_irq_handler,
	                IRQF_TRIGGER_LOW, RTC_NAME, NULL);
	if (r) {
...
MODULE_DESCRIPTION("MT6573 RTC Kernel Driver v3.4");
MODULE_LICENSE("GPL");
