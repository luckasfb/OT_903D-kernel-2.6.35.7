...
#define MIPI_RX_TX_REG (0xF7000000 + 0x26044)
...
/*----------------------------------------------------------------------------*/
static DEVICE_ATTR(config,     0664, jbd_show_config,  jbd_store_config);
static DEVICE_ATTR(count,      0664, jbd_show_count,   jbd_store_count);
static DEVICE_ATTR(trace,      0664, jbd_show_trace,   jbd_store_trace);
static DEVICE_ATTR(eint,       0664, jbd_show_eint,    jbd_store_eint);
static DEVICE_ATTR(cls,        0664, jbd_show_cls,     jbd_store_cls);
static DEVICE_ATTR(gain,       0664, jbd_show_gain,    jbd_store_gain);
...
int jbd_setup_eint(struct jbd_device *jb, struct jbd_eint_hlr *hlr)
{
		u16 temp1;
		u32 temp;
		temp1 =  *(volatile u16 *)MIPI_RX_TX_REG;	
		temp1 &=~(0x1<<12 | 0x1<<8);		//bit12 MIPI_RX_EN, bit8 MIPI_TX_EN
		*(volatile u16 *)MIPI_RX_TX_REG = (u16)temp1;
    ...
    mt_set_gpio_pull_select(GPIO_JBD_INPUT_UP_PIN, GPIO_PULL_UP);//GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_JBD_INPUT_LEFT_PIN, TRUE);
    mt_set_gpio_pull_select(GPIO_JBD_INPUT_LEFT_PIN, GPIO_PULL_UP);//GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_JBD_INPUT_RIGHT_PIN, TRUE);
    mt_set_gpio_pull_select(GPIO_JBD_INPUT_RIGHT_PIN, GPIO_PULL_UP);//GPIO_PULL_DOWN);
    mt_set_gpio_pull_enable(GPIO_JBD_INPUT_DOWN_PIN, TRUE);
    mt_set_gpio_pull_select(GPIO_JBD_INPUT_DOWN_PIN, GPIO_PULL_UP);//GPIO_PULL_DOWN);
