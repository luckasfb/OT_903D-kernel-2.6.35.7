...
struct msdc_dma {
	...
	u8  name[16];               /* dma channel name */
	struct resource *res;       /* dma resource */
};

struct mt6573_sd_host
{
	...
	u8                          reserved[2];
	u64                         starttime;
	u32                         error_address;  /* target address for mt6573_sd_retry() */
	u16                         card_rca;
};
