...
void jpeg_drv_dec_reset(void)
{
   ...
   /// Rey: for MT6516 E1...
   //REG_JPEG_DEC_INTERRUPT_STATUS = 0x80000000;
   //REG_JPEG_DEC_INTERRUPT_STATUS = 0xFFFFFFFF;
}
...
{
   /// Must add 16 bytes pre-fetch count and 3 for 4-bytes alignment
   REG_JPEG_DEC_FILE_TOTAL_SIZE = (size + 16 + 3) & 0xFFFFFFFC;
}
...
void jpeg_drv_dec_set_file_buffer(kal_uint32 buffer, kal_uint32 size)
{
    ...
    REG_JEPG_DEC_FILE_BRP = ((buffer + size + 3) & 0xFFFFFFFC);
