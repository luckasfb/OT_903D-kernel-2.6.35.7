...
extern struct mt6573_nand_host *host;
...
int find_empty_page_from_top(int start_addr,struct mtd_info *mtd)
{
	...
	for(page_offset=0;page_offset<(block_size/page_size);page_offset++)
	{
		...
		if(mtd->read_oob(mtd,(loff_t)current_add, &ops_pt)!=0)
		{
...
bool find_mirror_pt_from_bottom(int *start_addr,struct mtd_info *mtd)
{
	...
	for(mpt_locate=(block_size/page_size);mpt_locate>0;mpt_locate--)
	{
		...
		if(mtd->read_oob(mtd,(loff_t)current_start_addr, &ops_pt)!=0)
		{
...
int load_exist_part_tab(u8 *buf)
{
	...
	for(pt_locate=0;pt_locate<(block_size/page_size);pt_locate++)
	{
		...
		if(mtd->read_oob(mtd,(loff_t)pt_cur_addr, &ops_pt)!=0)
		{
	...
	if(pt_locate==(block_size/page_size))
	{
		...
		else
		{
			...
			mtd->read_oob(mtd,(loff_t)mirror_address, &ops_pt);
		}
	}
...
void part_init_pmt(struct mtd_info *mtd,u8 *buf)
{
	...
	int i=0;
	int err = 0;
	printk (KERN_INFO "mt6573_part_init_pmt  %s\n", __TIME__);
	...
	while (1)
	{
		...
		part->offset= lastblk;
		lastblk = part->offset+ part->size;
		printk (KERN_INFO "mt6573_part_init_pmt %llx\n",part->offset);
	...
	//retval=load_exist_part_tab(buf,mtd);  //not use buf now
      //for test
	//retval = ERR_NO_EXIST;
		...
		*(int *)sig_buf = MPT_SIG;
		...
		if(pageoffset!=0xFFFF)                                                      
		{
			...
			for(i=0;i<2;i++)
			{
				...
				ops_pt.datbuf = (uint8_t *)page_buf;
				if(mtd->write_oob(mtd,(loff_t)current_addr, &ops_pt)!=0)
				{
				...
				else
				{
					...
					//read back verify
					if((mtd->read_oob(mtd,(loff_t)current_addr, &ops_pt)!=0)||memcmp(page_buf,page_readbuf,page_size))
					{
						printk (KERN_INFO "new_pt read or verify first mirror page failed %x \n",current_addr);
						ops_pt.datbuf = (uint8_t *)page_buf;
						memset(page_buf,0,PT_SIG_SIZE);
						if(mtd->read_oob(mtd,(loff_t)current_addr, &ops_pt)!=0)
						{
