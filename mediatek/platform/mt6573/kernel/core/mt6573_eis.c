...
//
#define EIS_DEV_NAME            "mt6573-eis"
...
static int mt_eis_clk_onoff(__user unsigned long arg)
{
    ...
    if (arg == 1)
    {
        ...
        {
            hwEnableClock(MT65XX_PDN_MM_GMC1 , "EIS");
            hwEnableClock(MT65XX_PDN_MM_GMC1E , "EIS");
            hwEnableClock(MT65XX_PDN_MM_GMC1SLV , "EIS");
            hwEnableClock(MT65XX_PDN_MM_EIS , "EIS");
            bClockState = TRUE;
        }
    }
    else
    {
        if (bClockState == TRUE)
        {    
            hwDisableClock(MT65XX_PDN_MM_GMC1 , "EIS");
            hwDisableClock(MT65XX_PDN_MM_GMC1E , "EIS");
            hwDisableClock(MT65XX_PDN_MM_GMC1SLV , "EIS");
            hwDisableClock(MT65XX_PDN_MM_EIS, "EIS");
            bClockState = FALSE;            
        }
    }
...
static int mt_eis_ioctl(struct inode *inode, struct file *file,
...
MODULE_DESCRIPTION("MT6573 EIS driver");
MODULE_AUTHOR("ME3");
MODULE_LICENSE("GPL");
