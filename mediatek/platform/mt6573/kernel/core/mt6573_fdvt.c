...
#define FDVT_DEVNAME     "mt6573-fdvt"

#define MT6573FDVT_DEBUG
#ifdef MT6573FDVT_DEBUG
#define FDVT_FDVT_DEBUG(fmt, arg...) printk( "[MT6573FDVT] "  fmt, ##arg)
#else
...
#define FDVTPARMDB(fmt,arg...) printk("[MT6573FDVT]" "%s() - "  fmt, __FUNCTION__  ,##arg)
#else
...
{        
    ...
    iowrite32(0x01E101E1,FD_FNUM_0             );   
    iowrite32(0x02A10391,FD_FNUM_1             );   
    iowrite32(0x03F103D1,FD_FNUM_2             );   
    iowrite32(0x046103C1,FD_FNUM_3             );   
    iowrite32(0x01800180,VT_FNUM_0             );   
    iowrite32(0x021A02DA,VT_FNUM_1             );   
    iowrite32(0x0327030D,VT_FNUM_2             );   
    iowrite32(0x03800300,VT_FNUM_3             );   
...
static int mt_fdvt_clk_ctrl(int en)
{
    //
    if (en) {
        hwEnableClock(MT65XX_PDN_MM_FDVT, "FDVT");
    }
    else {
        hwDisableClock(MT65XX_PDN_MM_FDVT, "FDVT");
    }
...
static int MT6573FDVT_WaitIRQ(u32 * u4IRQMask)
{
    ...
    if (timeout == 0) {
        ...
        iowrite32(0x00030000, FDVT_START);
...
static __tcmfunc irqreturn_t MT6573FDVT_irq(int irq, void *dev_id)
{   
...
static int FDVT_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{   
    int ret = 0;
