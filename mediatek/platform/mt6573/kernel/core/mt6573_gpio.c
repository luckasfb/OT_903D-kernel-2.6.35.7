...
//#define  GIO_SLFTEST            
#define GPIO_DEVICE "mt6573-gpio"
...
#define CLK_NUM 6
static u32 clkout_reg_addr[CLK_NUM/2] = {
    (APCONFIG_BASE+0x160),
    (APCONFIG_BASE+0x164),
    (APCONFIG_BASE+0x168),
};
struct mt6573_gpio_obj {
    atomic_t        ref;
    dev_t           devno;
    struct class    *cls;
    struct device   *dev;
...
static struct mt6573_gpio_obj gpio_dat = {
    .ref  = ATOMIC_INIT(0),
    .cls  = NULL,
    .dev  = NULL,
...
static struct mt6573_gpio_obj *gpio_obj = &gpio_dat;
/*---------------------------------------------------------------------------*/
s32 mt_set_gpio_dir(u32 pin, u32 dir)
{
    u32 pos;
    u32 bit;
    struct mt6573_gpio_obj *obj = gpio_obj;
...
s32 mt_set_clock_output(u32 u4ClkOut, u32 u4Src)
{
    ...
    pinReg = clkout_reg_addr[u4ClkOut/2];
    regValue = INREG16(pinReg);
    if (u4ClkOut%2 == 0) {
        regValue &= 0xff00;  //clear low 8 bits. 
        u4Src &= 0xf;
        regValue |= (u4Src << 4);
    }else {
        regValue &=0x00ff;   //clear high 8 bits
        u4Src &= 0xf;
        regValue |= (u4Src << 12); 
    }
   
    OUTREG16(pinReg, regValue);
    return RSUCCESS;
}
...
{
    int idx;
    struct mt6573_gpio_obj *obj = (struct mt6573_gpio_obj*)dev_get_drvdata(dev);
...
/*---------------------------------------------------------------------------*/
//static DEVICE_ATTR(pin,      S_IWUGO | S_IRUGO, mt_gpio_show_pin,   mt_gpio_store_pin);
...
{
    struct mt6573_gpio_obj *obj = platform_get_drvdata(dev);
    int err;
