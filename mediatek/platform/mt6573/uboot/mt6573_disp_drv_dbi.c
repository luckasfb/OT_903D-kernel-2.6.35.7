...
static void init_lcd_te_control(void)
{
    ...
    // we will not enable tearing-free effect in uboot
    // because if lcm is not connected (.ex in meta mode), the phone will hang
    // and there is no high fps animation in uboot now.
    return;
