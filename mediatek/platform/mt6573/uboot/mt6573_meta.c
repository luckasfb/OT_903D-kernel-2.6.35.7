...
BOOL meta_check_board_trigger(void)
{
	//wait
	ulong begin = get_timer(0);
	printf("\n%s Check target meta boot\n",META_STR_MOD_PREF);
	printf("%s Wait 50ms for special keys\n",META_STR_MOD_PREF);
	
  while(get_timer(begin)<50)
  {    
		if(mt6573_detect_key(MT65XX_META_KEY))
		{	
			printf("%s Detect home key\n",META_STR_MOD_PREF);
...
BOOL meta_check_pc_trigger(void)
{
  ...
  mt6573_serial_set_current_uart(UART1);//added by Hong-Rong
...
//    if(meta_check_board_trigger())
//    {	return TRUE;
//    }
//	if(meta_check_pc_trigger())
//	{	
//	  return TRUE;
//	}
#endif
