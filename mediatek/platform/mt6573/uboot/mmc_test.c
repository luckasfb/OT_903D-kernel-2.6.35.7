...
#if defined(CONFIG_MMC) && defined(MMC_TEST)
...
#define MMC_TST_BUF_ADDR     (0x02000000)
#ifdef MMC_PROFILING
...
{
    printf("\t\tCount      : %ld\n", rpt->count);
    printf("\t\tMax. Time  : %ld counts\n", rpt->max_time);
    printf("\t\tMin. Time  : %ld counts\n", rpt->min_time);
    printf("\t\tTotal Size : %ld KB\n", rpt->total_size / 1024);
    printf("\t\tTotal Time : %ld counts\n", rpt->total_time);
    if (rpt->total_time) {
        printf("\t\tPerformance: %ld KB/sec\n", 
...
int mmc_prof_dump(int dev_id)
{
    ...
    printf("\tSD Host ID     : %d\n", dev_id);
    printf("\tOP Clock Freq. : %ld khz\n", host->clk / 1000);    
    printf("\tSD Clock Freq. : %ld khz\n", host->sclk / 1000);
    printf("\tCard Type      : %s card\n", (card->type == MMC_TYPE_MMC) ? "MMC" : "SD/SDHC");    
    printf("\tCard Size      : %ld MB\n", (card->nblks * card->blklen) / 1024 / 1024);
    printf("\tCard Max. Freq.: %ld khz\n", card->maxhz / 1000);
    ...
    if (total_read_time) {
        printf("\tPerformance Read : %ld KB/sec\n",
            ((total_read_size / 1024) * 32768) / total_read_time);
    }
    if (total_write_time) {
        printf("\tPerformance Write: %ld KB/sec\n",
...
int mmc_readback_blks(int dev_id, unsigned long addr, int blks, int bootarea)
{
    ...
    for (i = 0; i < blks; i++) {
        ...
        if (1 != mmc_block_read(dev_id, blknr + i, 1, (unsigned long*)buf)) {
