...
extern struct mt6573_CMD g_kCMD;
static pt_info pi;
...
void get_part_tab_from_complier(void)
{
	...
	while(mt6573_parts[index].flags!= PART_FLAG_END)
	{
		...
		lastest_part[index].size = mt6573_parts[index].blknum*BLK_SIZE ;
		lastest_part[index].offset = mt6573_parts[index].startblk * BLK_SIZE;
		lastest_part[index].mask_flags =  mt6573_parts[index].flags;  //this flag in kernel should be fufilled even though in flash is 0.
