...
#if 0   //TODO: figure out why this is happening; remove include of stat.h
    if (rv >= 0) {
...
static int write_block(MtdWriteContext *ctx, const char *data)
{
    ...
    while (pos + size <= (int) partition->size) {
        ...
        for (retry = 0; retry < 2; ++retry) {
            ...
            fprintf(stderr, "mtd: %s successfully wrote block at 0x%08lx\n", partition->name, pos);
            return 0;  // Success!
        }
...
int mtd_write_block_ex(MtdWriteContext *ctx, const char *data, off_t addr)
{
    ...
    off_t pos = lseek(fd, addr, SEEK_SET);
    if (pos == (off_t) -1) return 1;
